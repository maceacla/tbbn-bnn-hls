...
#include "ap_int.h"
#include "ap_float.h"
void r_128_196_4(ap_uint<1>data0[128], ap_uint<1>data1[784]){
	...
	for (int lidx0=0; lidx0<128; lidx0++){
	   ...
	   for (int lidx1=0; lidx1<196; lidx1++){
	      ...
	      ap_uint<1>val4=data2[(((lidx0*784)+(lidx1*4))+1)];
	      ap_uint<1>val5=data2[(((lidx0*784)+(lidx1*4))+2)];
	      ap_uint<1>val6=data2[(((lidx0*784)+(lidx1*4))+3)];
	      ap_uint<1>val7=data2[((lidx0*784)+(lidx1*4))];
	      acc0=(acc0+(((((((val3)^(val7)))!=(1))+((((val0)^(val4)))!=(1)))+((((val1)^(val5)))!=(1)))+((((val2)^(val6)))!=(1))));
	   }
	   float val8=data3[lidx0];
	   float val9=data4[lidx0];
	   data0[lidx0]=((((((acc0*val8)+val9))<(0.0)))!=(1));
	}
}