...
#include "ap_int.h"
#include "ap_float.h"
void r_5_16_4(ap_int<2>data0[5], ap_int<2>data1[64]){
	...
	for (int lidx0=0; lidx0<5; lidx0++){
	   ...
	   for (int lidx1=0; lidx1<16; lidx1++){
	      ...
	      ap_int<16>val4=data2[(((lidx0*64)+(lidx1*4))+1)];
	      ap_int<16>val5=data2[(((lidx0*64)+(lidx1*4))+2)];
	      ap_int<16>val6=data2[(((lidx0*64)+(lidx1*4))+3)];
	      ap_int<16>val7=data2[((lidx0*64)+(lidx1*4))];
	      acc0=(acc0+((((val3*val7)+(val0*val4))+(val1*val5))+(val2*val6)));
	   }
	   ap_int<16>val8=data4[lidx0];
	   float val9=data3[lidx0];
	   data0[lidx0]=((((((acc0+(val9*-1.0)))<(0.0))) ? -1.0 : 1.0)*val8);
	}
}